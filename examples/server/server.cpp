...
struct server_context {
    ...
    bool init_clip_model(common_params & params) {
        if (!params.mmproj.empty()) {
            multimodal = true;
            LOG_INF("Multi Modal Mode Enabled");
            const char * clip_path = params.mmproj.c_str();
            clp_ctx                = clip_model_load(clip_path, params.verbosity > 1);
            if (clp_ctx == nullptr) {
                LOG_ERR("unable to load clip model \"%s\" ", params.mmproj);
                return false;
            }
        ...
        if (multimodal) {
            ...
            if (n_embd_clip != n_embd_llm) {
                LOG_ERR(
                    "%s: embedding dim of the multimodal projector (%d) is not equal to that of LLaMA (%d). Make sure "
                    "that you use the correct mmproj file.\n",
    ...
    bool process_images(server_slot & slot) const {
        for (slot_image & img : slot.images) {
            ...
            img.image_embd_v.resize(n_tokens * n_embd);
            bool                 ok;
            clip_image_f32_batch batch_f32;
            ok = clip_image_preprocess(clp_ctx, img.img_data, &batch_f32);
            if (!ok) {
            ...
            LOG("slot id %d Encoding image\n", slot.id);
            ok = clip_image_batch_encode(clp_ctx, n_threads, &batch_f32, img.image_embd_v.data());
            if (!ok) {
    ...
    bool ingest_images(server_slot & slot, int n_batch) {
        ...
        while (idx < (int) slot.id2cache.size()) {
            ...
            decode_embd_batch batch_img(img.image_embd_v.data(), n_tokens, slot.n_past, 0);
    ...
    bool launch_slot_with_task(server_slot & slot, const server_task & task) {
        ...
        if (task.data.count("prompt") != 0) {
            slot.prompt = task.data["prompt"];
        } else {
            slot.prompt = "";
        }
            ...
            if (images_data != task.data.end() && images_data->is_array()) {
                ...
                for (const auto & img : *images_data) {
                    ...
                    if (!clip_image_load_from_bytes(image_buffer.data(), image_buffer.size(), img_sl.img_data)) {
                        LOG_ERR("slot %i - failed to load image [id: %i]\n", slot.id, img_sl.id);
                        return false;
                    }
                    LOG_INF("slot %i - loaded image\n", slot.id);
                    ...
                    while ((pos = prompt.find(pattern, pos)) != std::string::npos) {
                        ...
                        if (end_pos != std::string::npos) {
                            ...
                            try {
                                ...
                                LOG_ERR("Invalid image number id in prompt\n");
                                slot.images.clear();
                                return false;
                            }
                        }
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                    ...
                    if (slot.n_past == slot.n_prompt_tokens) {
                        ...
                        std::vector<llama_token> prefix_tokens =
                            has_images ? tokenize(slot.images[0].prefix_prompt, add_bos_token) : prompt_tokens;
                        for (; slot.n_past < (int) prefix_tokens.size(); ++slot.n_past) {
                            common_batch_add(batch, prefix_tokens[slot.n_past], slot.n_past, { slot.id }, false);
                        }
...
int main(int argc, char ** argv) {
    ...
    if (!ctx_server.init_clip_model(params)) {
        clean_up();
        LOG_ERR("%s: exiting due to multimodal loading error\n", __func__);
        return 1;
    }
