...
void setX(unsigned int X[], vector<unsigned char> m, int n) {
    n *= 64;
    for (int i = 0; i < 16; i++) {
        X[i] = (m[n + 4 * i + 3] << 24) + (m[n + 4 * i + 2] << 16) + (m[n + 4 * i + 1] << 8) + m[n + 4 * i];
    }
}
...
string md5(string text) {
    ...
    stringstream ss;
    //将A,B,C,D由大端格式变为小端格式
    ss << hex << (A & 0xffU) << ((A >> 8) & 0xffU) << ((A >> 16) & 0xffU) << ((A >> 24) & 0xffU) << (B & 0xffU)
       << ((B >> 8) & 0xffU) << ((B >> 16) & 0xffU) << ((B >> 24) & 0xffU) << (C & 0xffU) << ((C >> 8) & 0xffU)
       << ((C >> 16) & 0xffU) << ((C >> 24) & 0xffU) << (D & 0xffU) << ((D >> 8) & 0xffU) << ((D >> 16) & 0xffU)
       << ((D >> 24) & 0xffU);
    return ss.str();
}
